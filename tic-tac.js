...
function minmax(state, player, level) {
    ...
    if (possibleBoardState.length == 0 || level == 0) {
        ...
        for (var data in possibleBoardState) {
            if (player == AI_PLAYER) {

                var tempScore = getBoardState(possibleBoardState[data].board, player);
                if (tempScore == 10 || tempScore == -10) {

                    if (tempScore > bestScore) {
                        bestScore = tempScore;
                        bestState = new MinMaxState(possibleBoardState[data].position, possibleBoardState[data].state, bestScore);
                        console.log("BEST SCORE - after-> " + bestScore);
                    }
                    console.log("temp stop -> temp score" + tempScore + "--Best SCORE -->" + bestScore);
                    break;
                } else {
                    console.log("Calling mix max for the board - >>");
                    var minMaxState = minmax(possibleBoardState[data].board, opponent);
                    console.log("min max Score ---maximize -->" + minMaxState);
                    var score = minMaxState.score;
                    ...
                    if (score > bestScore) {
                        ...
                        //                        if (score == 10)
                        //                            break;
                        console.log("Now best score -->" + bestScore);
                        bestState = new MinMaxState(possibleBoardState[data].position, possibleBoardState[data].state, bestScore);
                    }
                }
            }
            console.log("Array for maximization --->>>>" + scoreArray);
            console.log("Maximization best state -->" + bestState);

            return bestState;
        } else {
            for (var data in possibleBoardState) {
                count++;
                console.log("Possible combination : " + count);
                console.log("Calling mix max for the board - >>");
                var tempScore = getBoardState(possibleBoardState[data].board, player);
                if (tempScore == 10 || tempScore == -10) {
                    ...
                    var minMaxState = minmax(possibleBoardState[data].board, opponent);
                    console.log("min max Score- minimization->" + minMaxState);
                    var score = minMaxState.score;

                    scoreArray.push(score);
                    console.log("**********Minimizing score*********");
                    if (score < bestScore) {
                        bestScore = score;
                        console.log("Now best score ->" + bestScore);
                        bestState = new MinMaxState(possibleBoardState[data].position, possibleBoardState[data].state, bestScore);
            ...
            console.log("Array for minimization ------->>>>>>" + scoreArray);
            //   return bestState;
            return new MinMaxState(bestScore, new Position(bestRow, bestCol));
        }
    }
